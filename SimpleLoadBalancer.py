...
class SimpleLoadBalancer(object):
    ...
    def update_lb_mapping(self, client_ip):
        ...
        if self.flow_table.has_key(client_ip) :
            ip_address, check_time = self.flow_table.get(client_ip)
            if current_time > check_time+2 :
                server_address = random.choice (self.server_arp_table.keys())
                self.flow_table[client_ip] = server_address, current_time
            else:
                self.flow_table[client_ip] = ip_address, current_time
        else:
            server_address = random.choice (self.server_arp_table.keys())
            self.flow_table[client_ip] = server_address current_time
    ...
    def update_client_arp_table(self, client_ip, client_mac, inport):
        if (self.client_arp_table.get(client_ip, (None,None)) != (client_mac,inport)):
            ...
            log.info("Client addrees: %s (MAC %s) Port: %s." % (client_ip, client_mac, inport))
    ...
    def send_proxied_arp_reply_to_server(self, arp_pkt):
        ...
        arpReply = arp()
        arpReply.opcode = arpReply.REPLY
        arpReply.hwdst = ETHER_BROADCAST
        arpReply.hwtype = arpReply.HW_TYPE_ETHERNET
        arpReply.prototype = arpReply.PROTO_TYPE_IP
        
        #Destination & Source setting
        arpReply.protodst = server_ip          
        arpReply.hwsrc = self.lb_mac           
        arpReply.protosrc = self.lb_ip        
        e_id = ethernet(type=ethernet.ARP_TYPE, src=self.lb_mac,
                     dst=ETHER_BROADCAST)
        e_id.set_payload(arpReply)

        # OpenFlow message assignment
        open_msg = of.ofp_packet_out()
        open_msg.actions.append(of.ofp_action_output(port = of.OFPP_FLOOD))
        open_msg.data = e_id.pack()
        open_msg.in_port = of.OFPP_NONE
        self.connection.send(open_msg)
    ...
    def forward_client_to_server(self, event, client_ip):
        ...
        server_address, time_taken = self.flow_table.get(client_ip)
        mac_address,port = self.server_arp_table.get(server_address)
        actions_array = []
        # set the address
        actions_array.append(of.ofp_action_dl_addr.set_dst(mac_address))
        actions_array.append(of.ofp_action_nw_addr.set_src(server_address))
        actions_array.append(of.ofp_action_output(port = port))
        
        # send the message
        msg = of.ofp_packet_out()
        msg.actions = actions_array
        msg.data = event.ofp
        msg.in_port = of.OFPP_NONE

        self.connection.send(msg)
       
        log.info("Server: %s forwarded packets to client: %s." % (server_address, client_ip))
